...
namespace Algorithms
{
    ...
    /// https://github.com/zoran-horvat/advent-of-code-2024/blob/main/Demo/Day06.cs
    ...
    internal static class Day06Alternate
    {
        ...
        private static IEnumerable<Point> GetObstructionPoints(this char[][] map, Position start)
        {
            ...
            foreach (Position current in map.Walk(start))
            {
                ...
                if (!map.Contains(ahead) || map.IsObstruction(ahead))
                {
                    continue;
                }

                if (!checkedObstructions.Add(ahead))
                {
                    continue;
                }

                if (!ahead.CausesLoop(map, current.TurnRight(), visited))
                {
                    continue;
                }
