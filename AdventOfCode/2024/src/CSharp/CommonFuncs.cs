...
namespace Advent2024
{
    internal static class CommonFuncs
    {
        ...
        public static IEnumerable<string> Antidiagonals(
...
#pragma warning disable IDE0305 // Simplify collection initialization
            new(
                data.Skip(startRow)
                    .Take(cols - startCol)
                    .Select((row, i) => row[startCol + i])
                    .ToArray()
            );
#pragma warning restore IDE0305 // Simplify collection initialization

        public static IEnumerable<string> Diagonals(
            this IEnumerable<string> data,
            int rows,
            int cols
        ) =>
            Enumerable
                .Range(0, cols)
                .Select(col => data.Diagonal(0, col, cols))
        ...
        public static List<int> ParseIntsNoSign(this string line) =>
            [
                .. Regex
                    .Matches(line, @"\d+")
                    .Select(static match =>
                        int.Parse(match.Value, CultureInfo.InvariantCulture.NumberFormat)
                    ),
            ];
        ...
        public static List<long> ParseLongsNoSign(this string line) =>
            [
                .. Regex
                    .Matches(line, @"\d+")
                    .Select(static match =>
                        long.Parse(match.Value, CultureInfo.InvariantCulture.NumberFormat)
                    ),
            ];
