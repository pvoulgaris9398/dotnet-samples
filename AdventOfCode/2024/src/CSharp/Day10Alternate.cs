...
internal static class Day10Alternate
{
    ...
    {
        List<string> data = testing
            ? TestDataSet2()
            : CommonFuncs.LoadFileData("..\\..\\..\\..\\..\\day10.txt");
        ...
        int distinctTrailScore = startingPoints.Sum(start =>
            map.Walk(start).CountDistinctHikingTrails(map)
        );
    ...
    private static int CountDistinctHikingTrails(
        this IEnumerable<((int row, int col) point, int count)> visitedPoints,
        char[][] map
    ) => visitedPoints.Where(node => map.At(node.point) == '9').Sum(node => node.count);

    private static int CountHikingTrails(
        this IEnumerable<((int row, int col) point, int count)> visitedPoints,
        char[][] map
    ) => visitedPoints.Count(node => map.At(node.point) == '9');

    private static IEnumerable<((int row, int col) point, int count)> Walk(
        this char[][] map,
    ...
    private static IEnumerable<(int row, int col)> GetUphillNeighbors(
        this char[][] map,
